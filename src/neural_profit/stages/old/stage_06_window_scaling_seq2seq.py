...
IN_ARTIFACT_GESTATION_WINDOW = Path(os.environ.get("IN_ARTIFACT_GESTATION_WINDOW", "reports/stage_03b_target_definition_summary.json"))
...
def load_json(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(obj, dict):
        raise TypeError(f"Se esperaba un dict JSON en {path}, pero llegó: {type(obj)}")
    return obj


def get_gestation_window_from_stage03b_summary(stage03b_env: Dict[str, Any]) -> Tuple[str, str]:
    """
    Obtiene (start_hhmm, end_hhmm) desde el envelope del stage_03b:
      details.gestation_window.start_hhmm
      details.gestation_window.end_hhmm

    Fallbacks soportados:
      - details.gestation_window_start / details.gestation_window_end
      - details.gestation_window.start / details.gestation_window.end
      - details.gestation_window (minute_of_day_min/max) => convierte a HH:MM
    """
    details = stage03b_env.get("details", {}) or {}
    gw = details.get("gestation_window", {}) or {}

    # 1) Ruta principal (la que usted ya tiene en el JSON)
    start = gw.get("start_hhmm")
    end = gw.get("end_hhmm")
    if start and end:
        return str(start), str(end)

    # 2) Fallbacks por nombres alternativos
    start = gw.get("start") or details.get("gestation_window_start")
    end = gw.get("end") or details.get("gestation_window_end")
    if start and end:
        return str(start), str(end)

    # 3) Fallback: minute_of_day_min/max
    ...
    raise KeyError(
        "No encontré la gestation_window en el summary envelope de stage_03b.\n"
        "Se esperaba:\n"
        "  details.gestation_window.start_hhmm y details.gestation_window.end_hhmm\n"
        "o al menos minute_of_day_min/max."
    )


def get_gestation_window_from_stage03b_file(path: Path) -> Tuple[str, str]:
    env = load_json(path)
    return get_gestation_window_from_stage03b_summary(env)
...
# ============================================================
def load_features_target(path: Path) -> Tuple[List[str], List[str], str, str]:
    if not path.exists():
    ...
    if not isinstance(features, list) or not features:
        raise KeyError("No se encontró 'details.schema.features' (lista no vacía) en el artifact.")
    if not isinstance(targets, list) or len(targets) < 2:
        raise KeyError("No se encontró 'details.schema.targets' con al menos 2 elementos en el artifact.")
    ...
    target_60 = str(targets[0])
    target_90 = str(targets[1])
...
def main() -> None:
    ...
    stage03b_env = load_json(IN_ARTIFACT_GESTATION_WINDOW)
    gestation_window_start, gestation_window_end = get_gestation_window_from_stage03b_summary(stage03b_env)

    log.info("[OK] Gestation window (from stage_03b): %s -> %s", gestation_window_start, gestation_window_end)
